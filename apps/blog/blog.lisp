...
(define-easy-handler (view-draft
		      ...
		      :uri (lambda (request)
			     (and (string= *ninx-blog-host* (header-in :host request))
				  (trivia:match
				      (str:split "/" (script-name* request))
				    ((list "" _ "view-draft" _) t)
				    (_ nil))))
		      :host *ninx-blog-host*) ()
  (trivia:match
...
(define-easy-handler (save-blog-post-as-draft
		      ...
		      :uri (lambda (request)
			     (and (string= *ninx-blog-host* (header-in :host request))
				  (trivia:match
				      (str:split "/" (script-name* request))
				    ((list "" _ "save-blog-post-as-draft" _) t)
				    (_ nil))))) ()
  (if (null (cookie-in "na-cookie"))
      (redirect "/admin/login")
      (trivia:match
...
(define-easy-handler (blog-page
		      ...
		      :uri (lambda (request)
			     (and
			      (string= *ninx-blog-host* (header-in :host request))
			      (trivia:match
				  (str:split "/" (script-name* request))
				((list "" _ _) t)
				(_ nil))))
...
(define-easy-handler (admin-dashboard
		      :acceptor-names '(ninx::ninx)
		      :host *ninx-blog-host*
		      :uri (define-matching-functions "/admin" *ninx-blog-host* request)) ()
